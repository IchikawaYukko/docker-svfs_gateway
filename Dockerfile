...
RUN		apk add fuse wget ruby pv curl bash rpm gcc linux-headers musl-dev && \
		...
		echo 'mount -t svfs -o segment_size=5120 /dev/svfs /mnt' >> /usr/local/bin/entrypoint.sh && \
		echo 'else' >> /usr/local/bin/entrypoint.sh && \
		echo 'mount -t svfs -o segment_size=5120,container=$CONTAINER /dev/svfs /mnt' >> /usr/local/bin/entrypoint.sh && \
